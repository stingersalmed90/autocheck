    ...
    private ResultadoCargaInventarioDTO procesarYGuardarInventarios(List<InventarioCargaDTO> inventariosDTO) {
        ...
        for (InventarioCargaDTO dto : inventariosDTO) {
            ...
            try {
                ...
                if (inventarioRepository.existsByChasis(dto.getChasis())) {
                    dto.setValido(false);
                    dto.setError("El chasis ya existe en el inventario");
                    resultado.setFilasConError(resultado.getFilasConError() + 1);
                    resultado.getErrores().add(dto);
                    continue;
                }
        ...
        if (resultado.isExitoso()) {
            resultado.setMensaje("Carga completada exitosamente. " + resultado.getFilasExitosas() + " vehículos registrados en inventario.");
        } else {
